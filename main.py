...
class AdbControllerApp(ctk.CTk):
    def __init__(self):
        ...
        self.title("Dars V3 Controller")
        ctk.set_appearance_mode("Dark")
        ctk.set_default_color_theme("blue")

        # Remove default title bar to create a custom one
        self.overrideredirect(True)
        self.is_maximized = False
        self.after(100, self.toggle_maximize)  # Start in maximized state
        ...
        self.grid_columnconfigure(0, minsize=500)  # Increased size for control panel
        self.grid_columnconfigure(0, weight=1)  # Expands to fill remaining space
        ...
        # --- Control Panel Setup ---
        self.control_panel = ctk.CTkFrame(self, corner_radius=15, fg_color="#242424")
        self.control_panel.grid(row=1, column=0, sticky="nsew", padx=20, pady=(0, 20))
        ...
        ctk.CTkLabel(self.control_panel_scrollable, text="Dars V3 Controller", font=ctk.CTkFont(size=30, weight="bold"),
                     text_color="#18a2ff").grid(
            row=0, column=0, pady=(20, 10), sticky='ew', padx=25)

        ctk.CTkFrame(self.control_panel_scrollable, height=2, fg_color="#4a4a4a").grid(row=1, column=0, sticky='ew',
                                                                                       padx=25,
                                                                                       pady=15)
        ...
        self.device_listbox = tk.Listbox(self.control_panel_scrollable, height=6, font=("Segoe UI", 12),
        ...
        self.tab_view = ctk.CTkTabview(self.control_panel_scrollable, segmented_button_selected_color="#1f6aa5",
        ...
        self.status_label = ctk.CTkLabel(self.control_panel_scrollable, text="", anchor='w',
                                         font=("Segoe UI", 14, "italic"))
        self.status_label.grid(row=5, column=0, sticky='ew', padx=25, pady=(10, 0))

        # --- Device View Panel Setup ---
        self.device_view_panel = ctk.CTkFrame(self, fg_color="#1e1e1e", corner_radius=15)
        self.device_view_panel.grid(row=1, column=1, sticky="nsew", padx=(0, 20), pady=(0, 20))
    ...
    def _create_title_bar(self):
        """Creates a custom title bar with app title and control buttons."""
        self.title_bar = ctk.CTkFrame(self, fg_color="#1f1f1f", corner_radius=0)
        self.title_bar.grid(row=0, column=0, columnspan=2, sticky='ew')
        self.title_bar.grid_columnconfigure(0, weight=1)

        title_label = ctk.CTkLabel(self.title_bar, text="Dars V3 Controller", font=ctk.CTkFont(size=14, weight="bold"))
        title_label.grid(row=0, column=0, sticky="w", padx=10, pady=5)

        # Close Button
        close_button = ctk.CTkButton(self.title_bar, text="X", command=self.close_window, width=30, height=30,
                                     fg_color="#dc3545", hover_color="#c82333", corner_radius=5)
        close_button.grid(row=0, column=3, padx=(5, 5))

        # Maximize/Restore Button
        self.maximize_button = ctk.CTkButton(self.title_bar, text="▢", command=self.toggle_maximize, width=30,
                                             height=30,
                                             fg_color="#3a3a3a", hover_color="#505050", corner_radius=5)
        self.maximize_button.grid(row=0, column=2, padx=(5, 0))

        # Minimize Button
        minimize_button = ctk.CTkButton(self.title_bar, text="-", command=self.minimize_window, width=30, height=30,
                                        fg_color="#3a3a3a", hover_color="#505050", corner_radius=5)
        minimize_button.grid(row=0, column=1, padx=(5, 0))

        # Bind title bar for dragging the window
...
---
 ...
 ADB Path: Ensure that ADB (Android Debug Bridge) is installed and added to your system's PATH for the application to function correctly.
    ...
    def create_device_frame(self, serial):
        ...
        canvas_container = ctk.CTkFrame(device_frame, fg_color="#1e1e1e", corner_radius=10)
        ...
        canvas = tk.Canvas(canvas_container, bg="#1e1e1e", highlightthickness=0)
        canvas.place(relx=0.5, rely=0.5, anchor=tk.CENTER)  # Use .place for custom sizing
    ...
    def on_canvas_container_resize(self, event):
        ...
        # Calculate new canvas size to maintain aspect ratio (9:16)
        aspect_ratio = 9 / 16
    ...
    def send_adb_tap(self, event, serial):
        try:
            ...
            image_x_offset = (canvas.winfo_width() - effective_width) // 2
            image_y_offset = (canvas.winfo_height() - effective_height) // 2
